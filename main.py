...
def printGrid(grid, grid_pieces, x, y):
    y = (y_ext-1-y) % y_ext
    for x0, row in enumerate(grid):
        for y0, item in enumerate(np.flip(row)):
            if x==x0 and y==y0:
                print('_ ', end='')
                continue
            if (x0, (y_ext-1-y0) % y_ext) in grid_pieces:
                print('0 ', end='')
                continue
...
if os.path.exists(setup_file) and use_cache_setup:
    ...
    setup_imgs = ['top_left_corner.jpg', 'left_side_start_at_9.jpg', 'top_start_at_6.jpg']
...
setup_config = {   
    ...
    # start left_side_start_at_9.jpg
    26: (14, 0, 0),
    27: (13, 0, 0),
    28: (12, 0, 0),
    29: (11, 0, 0),
    30: (10, 0, 0),
    31: (9, 0, 0),
    # end left_side_start_at_9.jpg
    
    # start top_start_at_6.jpg'
    32: (0, 6, 0),
    33: (0, 8, 0),
    34: (0, 7, 3),
    35: (0, 14, 0),
...
# set some as manually solved
grid_solved[12, 31] = True
grid_solved[14, 31] = True
grid_solved[14, 30] = True
grid_solved[13, 30] = True
grid_solved[12, 30] = True
...
y = 26
del grid_pieces[(x, y)]
grid_solved[x, y] = False

printGrid(grid_solved, grid_pieces, x, y)
...
for x in range(grid_solved.shape[0]):
    for y in range(grid_solved.shape[1]):
        #if x != 13 or y != (y_ext-4-1) % y_ext: continue
